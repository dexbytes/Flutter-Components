...
class BubbleClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    switch (nip) {
      ...
      case BubbleNip.centerTop:
        path.addRRect(RRect.fromLTRBR(
            0, size.height - _startOffset, size.width, _endOffset, radius));
        break;
      case BubbleNip.rightCenter:
        path.addRRect(RRect.fromLTRBR(
    ...
    if (showNip) {
      switch (nip) {
        ...
        case BubbleNip.centerTop:
          ...
          final path2 = Path()
            ..moveTo(size.height - _startOffset, cy - nipHalf)
            ..lineTo(size.height - _startOffset - radiusY, cy - nipHalf)
            ..lineTo(size.height - _startOffset - radiusY, cy + nipHalf)
            ..lineTo(size.height - _startOffset, cy + nipHalf);

          if (nipRadius == 0) {
            path2.lineTo(size.height, cy);
          } else {
            path2
              ..lineTo(size.height - _nipPY, cy + _nipPX)
              ..arcToPoint(
                Offset(size.height - _nipPY, cy - _nipPX),
                radius: Radius.circular(nipRadius),
                clockwise: false,
              );
          }
